# Runs /implement + /standards-review for the spec 319 ECR-slip tickets ("a store day's slips at the day close", web half of BackOffice spec 2030) in fresh Claude Code sessions,
# sequentially, fully AFK.
#
#   .\run-implements-spec319.ps1                  (320, 321, 322, 323)
#   .\run-implements-spec319.ps1 -Tickets 322,323
#   .\run-implements-spec319.ps1 -DryRun          (pre-flight + plan only, starts nothing)
#   .\run-implements-spec319.ps1 -SkipReview      (implement rounds only)
#   .\run-implements-spec319.ps1 -SmokeTest       (one trivial session through the real harness)
#
...
#
# ---------------------------------------------------------------------------------------
# Dependency map (319 spec; nothing of this wave has landed; BackOffice 2034 + 2035 are DONE (merged da71a9621 on pricing2)):
#   320 --> 321 --+-- 322
#                 \-- 323
#   BackOffice 2034 feeds 320 + 321; BackOffice 2035 feeds 321, 322, 323 (both done, cross-repo)
#
# Running order:
#   * Exactly topological. 320 goes first on purpose anyway: it adds the envelope-keeping read to
#     src/core/api.ts and the ONE shared AttachmentWeb/Access probe query that 321-323 all read -
#     a shared-surface change every later slice builds on.
#   * 321 before either action: it owns the drawer component and storeDayOwnerKey().
#   * 322 before 323 (siblings): 322 puts Add slip and the in-flight dismiss guard into the drawer's
#     frame; 323 only adds a per-row Withdraw and its own dialog, onto a frame that is then final.
#   * No slice is backend-blocked: every door is shipped and both Web contracts are recorded, so all
#     four prove against Playwright stubs. Nothing needs to be pushed to the end.
#
# Preconditions (the script checks 1, 2 and 4 - you own 3 and 5):
#   1. Nothing in this repo blocks it. (BackOffice 2034/2035 are done; each session re-checks that first.)
...
#      Expected for this wave: spec319 (worktree C:\Playground\oms-react-spec319)
...
#   5. Run it FROM THE WORKTREE C:\Playground\oms-react-spec319, never from the main checkout.
#      A live SIS.Api on :5111 is NOT required - every slice stubs CollectionWeb/* and
#      AttachmentWeb/* at Playwright, shaped exactly as BackOffice 2034/2035 record them.
param(
    [int[]]$Tickets = @(320,321,322,323),
    [string]$Model = "opus",
...
Say "Branch: $branch   (expected for this wave: spec319 (worktree C:\Playground\oms-react-spec319))" "DarkGray"
...
3. Wave-specific facts - read these before you code:
   - The spec is .issues\319-ecr-slips-web-half-spec.md and the wayfinder map is
     C:\Work\DMSCO\BackOffice\.issues\2030-ecr-slips-at-the-day-close-spec.md (the governing BackOffice spec; there is no wayfinder map). Read the spec AND your ticket.
     No ticket of this wave had landed when this run began; read the committed diffs of the
     ones that ran before yours in THIS run (git log on spec319) before re-deciding anything they settled.
   - SCOPE WALLS. No npm dependency, no route, no menu leaf, no grant, no new area or feature folder.
     Everything lands in features\collection\inquiry (both grids that open the drawer live there)
     plus the one core read below. A slice that seems to need a package is a BLOCKER.
   - THE DRAWER PRIMITIVE (321). There is no shared drawer/sheet. The call-center ConfirmSheet is
     another feature's and must NOT be imported. Do not put one in layout\ either - features may only
     import @/core. Build the drawer inside collection\inquiry the way AssignmentUploadDialog is
     built, and record that choice in 321's comments. Do not invent a src\components\ folder.
   - THE CORE READ (320). Add ONE envelope-keeping read to src\core\api.ts, built from the existing
     send / readEnvelope / failureFromEnvelope / businessFromEnvelope, so it has the same base,
     X-Web-Client header, 401 redirect and ApiError taxonomy as api.get. It is GENERIC over the
     sibling fields: core must never name slipCountsUnavailable or withdrawn. api.get and every other
     caller keep their behaviour. Record the chosen name in 320's comments; 321 uses the same read for
     ByOwner (withdrawn sits beside data).
   - THE PROBE (320). GET AttachmentWeb/Access is read under ONE shared TanStack query key defined in
     320 and reused by 321-323 - never a second fetch, never a per-component copy. Pending, refused
     (503 NOT_SET_UP, 403, network) or malformed = HIDDEN. Membership is Array.isArray(x) and
     x.includes('CASH_CLOSE'); a bare string "CASH_CLOSE" must read FALSE (String.includes would
     pass it - that is the trap). categories gates column + filter + drawer + Add; withdrawCategories
     (typed optional string[] in 320) gates Withdraw only, and only 323 reads it.
   - OLD DRIVES STUB NO PROBE. ready-drive, collection-drive, collections-filters-drive and
     four-filters-drive never stub AttachmentWeb/Access, so the probe fails and the column must be
     hidden: they should pass UNMODIFIED. If one breaks, the fail-closed path is wrong - fix the code,
     not the old drive.
   - NULL IS UNKNOWN. slipCount null => a dash, not clickable, never in "No slip" (=== 0 only; never
     == 0, never !x). cardTotal null => a dash, never 0.000. businessDay is string | null on BOTH row
     models (src\core\models\collection.ts): a null businessDay never builds an owner key and never
     opens the drawer. Collections settlement rows are always null.
   - CARD TOTAL. Only Ready gains cardTotal (number | null), formatted like its money neighbours and
     under the currency header on the same terms. Collections ALREADY has a numeric cardTotal - leave
     it alone. Collections slipCount is keyed by each row's own businessDay: never merge or sum the
     rows of one multi-shift receipt.
   - OWNER KEY. One pure storeDayOwnerKey(storeId, businessDay) in 321 => storeId + '/' + yyyy-MM-dd,
     by STRING handling of businessDay only. Never new Date(), toLocale*, or Intl. 322 and 323 import
     it; they never re-spell the key.
   - TIMESTAMPS. storedAt and withdrawnAt are local wall clock with no zone. Render the string as sent
     (a pure string cut of the T separator is acceptable); never parse either through Date.
   - BYTES. /Content goes through api.blob; Add goes through api.upload with a FormData the FEATURE
     builds (core never learns part names). The shared send uses credentials same-origin by design -
     the contract's 'include' is satisfied; do NOT change send. Revoke every object URL on selection
     change, on drawer close, and (323) when the previewed slip is withdrawn. Download saves the
     already-fetched blob under fileName - no second fetch.
   - THE TILL LABEL. sourceDevice when non-empty, else the "Web <sep> uploadedBy" label. The ticket
     spells the separator as a middle dot (U+00B7): copy it from ticket 321 into the i18n value and
     interpolate uploadedBy as a named param. 2035 supersedes 2034 here (2034 says plain "Web").
   - RETRY (322) branches on apiErrorCode / apiErrorKind, NEVER on statusCode. Know the core taxonomy
     before writing isRetryable: fetch throwing => kind network; a CODED 5xx envelope => kind business
     WITH its code (so NOT_SET_UP and FILE_SERVER_UNREACHABLE both arrive as business 503s and differ
     only by code); a non-JSON answer => kind server/unknown with NO code. Beware a bare 403 with no
     body also lands as kind unknown with no code - it is an auth refusal, not a flaky transport, so
     do not offer a retry for it; log that ruling to HITL. ClientRequestId is minted once per picked
     file, reused on every retry, and survives a drawer close/reopen while in flight (the drawer also
     refuses dismissal while anything is in flight).
   - WITHDRAW ANSWERS (323). 403 has NO body (core maps it to kind unknown, statusCode 403, no code):
     that is the ONE place a status branch is correct, and it removes the action for this drawer. 400
     codes are the field names reasonCode / note - show message and keep the input. 404 NOT_FOUND:
     say so, close, re-read ByOwner. 503 NOT_SET_UP: message, no retry. A repeat 200 on an
     already-withdrawn row is a plain success. Disable confirm while in flight.
   - GRID FRESHNESS. After Add or Withdraw, re-read ByOwner and INVALIDATE the grid's query without
     refetching it under the user (e.g. refetchType none). Ready's key starts ['collection','ready'],
     Collections' starts ['collection','collections'].
   - WORDING - A REVERSAL FROM SPEC 308. The web still has only the en bundle (namespace collection,
     src\locales\en\collection.json; put this wave's keys in ONE new slips group there, no new
     namespace). BUT: (a) server refusal messages are shown exactly as sent, English then Arabic in
     one string, never re-translated; (b) 323's five reason labels go under their own keys with the
     English AND the Arabic in the VALUE, copied byte-for-byte from the table in ticket 323 (UTF-8).
     The Withdrawn list shows the server's reasonLabel / reasonLabelArabic, never those keys. Those
     labels are DRAFTED and wait on the owner's read - do not reword them.
   - SETTLED - DO NOT RE-LITIGATE. No count cap on Add (C9), no photo downscaling (S3), no restore or
     undo (C7), no collection cutoff (C3: Add/Withdraw work the same on Ready and collected rows), no
     server-side slip filter (F3), no preview or download of a withdrawn slip (C6). The 10,485,760-byte
     edge passes; +1 refuses.
   - THE CONTRACTS LIVE IN THE OTHER REPOSITORY: C:\Work\DMSCO\BackOffice (branch pricing2, spec
     2030 merged as da71a9621). The envelope shapes are under '## Web contract' in
     .issues\2034-ready-and-collections-show-each-store-days-slip-count.md and
     .issues\2035-finance-withdraws-a-wrong-slip-and-it-stops-being-readable.md - 2035 WINS where they
     differ. If that checkout is not on pricing2 when you look, read with git -C <repo> show
     pricing2:<path> (read-only). You may READ its .issues and source. You may NOT edit, stage,
     commit, switch branch, build, test or run anything there - it has its own loop and reviewer.
   - FIRST ACT OF EVERY TICKET: confirm BackOffice 2034 and 2035 are 'status: done' and carry
     '## Web contract'. If not, that is a BLOCKER: log it and stop with AFK-BLOCKED.
   - Cross-check the contract against the committed code spec 319 names (CollectionReadyRowModel.cs,
     CollectionInquiryModel.cs, SlipCountedResponse.cs, Services\SIS.Api\Endpoints\Attachments\
     AttachmentWebEndpoints.cs, Services\SIS.Api\Endpoints\Pos\CollectionWebEndpoints.cs). If they
     disagree, follow the CODE (it is what will be served), log it to HITL, and record the drift in
     your ticket's comments. Stub EXACTLY that shape; never invent a field or a part - a screen built
     on a guessed field fails silently the day the door answers.
...
6. How to verify - a green typecheck is NOT proof a screen works:
   ...
   - Pre-existing baseline for this wave, measured when this runner was generated -
     ...
     * npm test: 143 files / 2455 tests, ALL PASSING. Any red test is yours.
     * npm run lint: all three gates CLEAN (boundaries 642 files, contrast 129 pairs, colour literals).
     * Measured 2026-09-26 on branch spec319 at 2978666, before any ticket of this wave.
7. Proof checkboxes marked OWNER, manual-smoke, or needing a live backend are NOT yours: leave them
   ...
   that is the live walk against a real SIS.Api with the File Server key set (none is up; every drive is stubbed), and the owner's read of the drafted EN+AR reason labels (323) and every new string. The ticket may still complete AFK with those open; every OTHER
...
"@
    ...
    if ($leftBehind) {
        ...
        Say "    .\run-implements-spec319.ps1 -Tickets $(($Tickets | Where-Object { $_ -ne $t }) -join ',')" "Yellow"
        exit 9
    }
...
You are reviewing ONLY the diff introduced by ticket $t of spec 319: git diff $baseSha...HEAD.
...
- The wave's own rules to check against:
  no new npm dependency, route, menu leaf, grant or feature folder - all in features/collection/inquiry
  plus one core read; every server field or form part used appears in BackOffice 2034/2035's Web
  contract or the committed SIS.Api code - an invented field is SERIOUS; the new core read is generic
  and names no feature field, and nothing hand-rolls fetch or changes send; the Access probe is ONE
  shared query, fails closed on pending/refused/malformed, and tests array membership (a bare string
  must fail); null slipCount/cardTotal render a dash, never 0, are never clickable and never match
  "No slip" (=== 0 only); a null businessDay never builds a key; the owner key is string-built in one
  function 322/323 reuse; timestamps are never parsed through Date; object URLs are revoked on change,
  close and withdraw; retry branches on the error code/kind, never the status (NOT_SET_UP never
  retries) and reuses the per-file ClientRequestId; Withdraw is gated on withdrawCategories and its
  bare 403 removes the action; grid queries are invalidated, not refetched under the user; reason
  labels live under their own keys and the Withdrawn list shows the server's labels; the old sibling
  drives pass unmodified; nothing in the BackOffice repo was touched; stubbed Proof boxes are honest.
...
Write-Host ""
Say "Spec 319 wave complete. Read .afk\REVIEW-*.md and .afk\HITL-*.md before trusting the results." "Green"
Say ">>> Nothing here is live until OMS DB script 003, the attachments seed, SIS.Api and the till deploy; until the File Server key exists (BackOffice 1962) the probe answers 503 NOT_SET_UP and every slip surface stays hidden - correct, not a bug. <<<" "Yellow"
Say ">>> Drive both grids and the drawer once against a real SIS.Api after that deploy; tonight every proof was stubbed against the BackOffice 2034/2035 Web contracts. <<<" "Yellow"
Say ">>> The owner still owes a read of the drafted EN+AR reason labels (323) and of every new string in the collection bundle. <<<" "Yellow"
Say ">>> Merge back: from C:\Playground\oms-react run git merge --ff-only spec319 (rebase spec319 onto main first if main moved). Copy .afk\HITL-*.md and .afk\REVIEW-*.md out BEFORE any git worktree remove. <<<" "Yellow"
