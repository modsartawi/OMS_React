# Runs /implement + /standards-review for the spec 289 bonded-return screen tickets ("the return the web cannot yet create") in fresh Claude Code sessions,
# sequentially, fully AFK.
#
#   .\run-implements-spec289.ps1                  (290, 291, 292, 293, 294)
#   .\run-implements-spec289.ps1 -Tickets 292,293
#   .\run-implements-spec289.ps1 -DryRun          (pre-flight + plan only, starts nothing)
#   .\run-implements-spec289.ps1 -SkipReview      (implement rounds only)
#   .\run-implements-spec289.ps1 -SmokeTest       (one trivial session through the real harness)
#
...
#
# ---------------------------------------------------------------------------------------
# Dependency map (289 spec; nothing landed yet - 290 is the first slice; 295 is EXCLUDED):
#   290 --> 291 --+--> 292 --+--> 294 -->  (295, excluded from this run)
#                 \--> 293 --/
#
...
#
# Running order:
#   * The order is exactly the blocked-by topological order, with one tie broken deliberately.
#   * 292 and 293 are SIBLINGS - both blocked only by 291, and neither blocks the other. They are
#     serialized 292 then 293 because they edit the same component (ReturnDialog), one panel each.
#     Either order is correct; running them concurrently is not. 293 runs second and must NOT
#     restructure the reason cards or address panel that 292 just built.
#   * 290 runs first on its own merit as well as its edges: it is the only slice that FIXES A BUG
#     THAT HAS SHIPPED (the 'BB' gate offers Return at SMSA stores, where a return persists, books
#     no pickup and never refunds - silently), and it deletes a gate that a later slice's screen
#     sits on top of.
#   * 295 is NOT in this wave. Its proof needs a live SIS.Api and BackOffice spec 1283's own
#     tickets to have shipped, so it cannot run unattended - a session would burn the night trying
#     to make an unbuilt endpoint answer. It is the wave's drift detector and is run by hand.
#
# Preconditions (the script checks 1, 2 and 4 - you own 3 and 5):
#   1. Nothing outside this wave blocks it - 290 is the first slice and starts from a clean branch.
...
#      Expected for this wave: spec/289-bonded-return
...
#   5. A live SIS.Api is NOT required and must not be waited for - every slice in this wave is
#      proven against checked-in fixtures with the network stubbed at Playwright. The screen's
#      layout is already approved as an HTML build target (named in the briefing); the loop
#      follows it and does not re-decide the arrangement.
param(
    [int[]]$Tickets = @(290,291,292,293,294),
    [string]$Model = "opus",
...
Say "Branch: $branch   (expected for this wave: spec/289-bonded-return)" "DarkGray"
...
3. Wave-specific facts - read these before you code:
   - The spec is .issues\289-bonded-return-screen-spec.md and the wayfinder map is
     C:\Work\DMSCO\BackOffice\.issues\1266-the-return-order-the-web-cannot-yet-create.md. Read the spec AND your ticket.
     Nothing in this wave has landed yet - 290 is the first slice, so there are no sibling
     diffs to read. Every LATER session must read the committed diffs of the tickets ahead of it
     before re-deciding anything they settled.
   - This wave adds NO npm dependency, NO new feature folder, NO new route and NO menu entry. It
     fills in an existing placeholder command on an existing screen (src/features/oms/document).
     If a slice looks like it needs a dependency, that is a BLOCKER, not a 3am decision.
   - THE TWO SERVER FIELDS DO NOT EXIST YET. `canReturn` on the header model and `returnedQuantity`
     on the line model are BackOffice spec 1283 section 2b additions that have not been built. Add
     them as OPTIONAL fields, and make ABSENT read as NOT RETURNABLE - fail closed, never enabled.
     The five captured live payloads in src/features/oms/document/__fixtures__/payloads.ts carry
     neither field; commands.test.ts drives off them, and they must STAY green as exactly that
     fail-closed proof. Do NOT edit those captures to add the fields.
   - `returnedQuantity` IS A GUESS, AND MUST STAY ONE. BackOffice spec 1283 section 2b says the
     line gains "a returned-so-far (or remaining) quantity" and never names the field. Ticket 295
     confirms it against a live door. Do not rename it to something that reads better, do not flip
     it to a remaining-quantity spelling, do not "correct" it. Renaming it here silently removes
     the one gate this wave has on the transcription.
   - NO AMOUNT EVER CROSSES THE WIRE - no price, discount, VAT, fee or total, in either direction.
     Line money is DISPLAYED read-only as context and is absent from the request body. That body
     will look incomplete beside the line model and beside the WPF's - that IS the design: a
     client-supplied refund figure is structurally impossible rather than validated against.
     Adding unitPrice/vatAmount/discount "for completeness" destroys the property the whole door
     exists for, and every gate stays green while you do it.
   - THE SILENT-ZERO TRAP (ticket 293). A header condition's money is `condAmount`. Its `condValue`
     at line 0 is structurally .000, so reading condValue gives a fee that displays as costing
     nothing - no exception, green suite, money that never moves. Also: one ticked fee exists as
     the item-0 row AND as one distributed copy per line, so summing both charges it twice. Take
     the item-0 row alone and read condAmount.
   - Ticket 290 DELETES A GATE THAT AN EXISTING DRIVE ASSERTS. tools/document-actions-drive.mjs
     item 6 asserts the 'BB' gate, and its header comment explains synthesising a 'BB' payload
     because no capture carries one. Update that drive IN PLACE as part of 290. A session that
     only writes the new drive leaves a red one behind and calls the slice done.
   - After 290, `grep -r "BZ02" src/` and a search for the 'BB' literal must both come back EMPTY.
     Neither may exist in this repo. The store rule lives on the server and arrives as one boolean.
   - THE FOLLOWING ARE RULED, NOT OPEN. Each was argued and settled, and each is something a
     thoughtful session will want to re-decide at 3am. Re-deciding one is a review finding:
     * DIALOG, not a route. The two grids are STACKED, not tabbed.
     * NEITHER reason card is pre-selected. Do not default to "Return and refund" as the common
       case: a pre-selected radio is exactly how an irreversible refund gets clicked through.
     * Under Refund Only the address panel is REMOVED FROM THE TREE. Not disabled, not greyed,
       not collapsed - absent.
     * NO grand total anywhere. The screen will look incomplete; leave it incomplete. The server
       recomputes VAT and discount pro-rata, so any total here is a number the client invented
       and an operator would quote to a customer.
     * NO box count and NO total weight fields. The WPF requires both and then sends neither.
     * Fees open UNTICKED and have NO select-all. Select-all is on lines only.
     * A `replayed: true` answer renders as PLAIN SUCCESS with one extra clause - never an error.
     * A refusal keeps the dialog OPEN with every selection intact, behind a banner that stays
       after the toast has gone.
     * `requestId` is minted once per dialog OPENING and kept across retries - NOT per submit
       press. A fresh key per press creates a second return, the exact failure the key prevents.
     * On success the screen STAYS PUT and reloads; it does not navigate to the created return.
     * There is NO pre-confirm. The dialog IS the confirmation, and core/services/confirm is not
       used on this path.
   - THE LAYOUT IS APPROVED AND GOVERNS. It is a self-contained HTML page with live toggles - open
     it and build against it rather than inventing a reasonable-but-different arrangement:
     C:\Work\DMSCO\BackOffice\.issues\assets\1270-return-screen-build-target.html
   - i18n: every new key goes under `returnDocument.*` in the EXISTING `document` namespace. No new
     namespace and no core/i18n.ts change. Two keys are DELETED, each in its own slice:
     `command.disabled.beyondBorderOnly` (ticket 290) and `returnDocument.unavailable` (291).
     The word "close" must NEVER appear on this screen - CONTEXT.md reserves it for cancelling a
     document, and a return is not a cancellation.
   - REUSE, do not build: Modal (already gives the wide max-width, the internally scrolling body
     and the pinned footer slot), ErrorBanner (already the banner shape, and it takes children),
     notify, and the cached SdDocument/Districts read in core/services/lookups (already fetched
     for the Change Store picker, so the district picker is free - and a feature may never import
     another feature). Modal's footer is justify-end, so the submit bar's gate sentence rides on
     me-auto, exactly as the build target does.
   - Ticket 293 adds the delivery-fee CATEGORY CODE to this repo as a display constant. That is a
     considered exception, ruled in spec 289 D4 - not a slip. Unlike BZ02, which is a value a
     running program branches on to decide whether money moves, this one only decides which rows
     are DRAWN: the server re-reads the rate for every type it is given, and a wrong filter is
     visible on screen the instant it is wrong. Carry that reasoning as a comment at the constant.
     Do NOT read it as licence to reintroduce BZ02 or 'BB' anywhere.
   - The submit bar states ONE missing thing at a time, in the order the operator must act - select
     lines, then quantity, then reason - and then flips to a plain summary. It is not a list of
     complaints. The gate returns a KEY and its parameters, never a sentence; t() lives at the
     call site.
   - Fully-returned lines are HIDDEN, and the grid header says how many were hidden. An operator
     must never have to wonder whether a missing line is missing or filtered.
   - The governing frontend spec (289) lives in THIS repo, but it TRANSCRIBES its wire contract
     from BackOffice spec 1283 in the OTHER repository (C:\Work\DMSCO\BackOffice). You may READ
     1283, map 1266, ticket 1270's build target and anything they name. You may NOT edit, stage,
     commit or run anything in that repository - it has its own branch, its own loop and its own
     reviewer. If you believe something there is wrong, log it to the HITL doc and carry on.
   - The create door is NOT BUILT YET. Its agreed shape, transcribed in spec 289 under the single
     provenance line "From BackOffice spec 1283 sections 2 and 2b", is exactly:
       POST SdDocumentWeb/CreateReturn
       { requestId, refDeliveryNo, reason: 'RTRF' | 'RF',
         lines: [ { lineNumber, itemNumber, quantity } ],
         conditionTypes: string[],
         shippingAddress?: { street1, street2, cityCode, cityName, districtCode, districtName,
                             postalCode, buildingNumber, shortAddress, gpsLat, gpsLon },
         note? }
       200  -> { documentNo, orderNo, documentReason, storeCode, replayed }
       400  -> the envelope with success:false and errors[0].errorCode
     Stub THAT shape at Playwright to prove the screen. Do not invent fields, do not add an amount,
     do not soften it to whatever is convenient - a screen built against a guessed shape fails the
     day the endpoint lands, and it fails silently on exactly the fields you guessed.
   - The screen branches on NO error code at all. Spec 1283 mints two and calls their values build
     detail; render whichever code arrives, and never hard-code one.
...
5. How to verify - a green typecheck is NOT proof a screen works:
   ...
   - Pre-existing baseline for this wave, measured when this runner was generated -
     ...
     * npm test: CLEAN - 119 files, 1909 tests, all passing. Any failure you see is yours.
     * npm run lint: all three gates CLEAN (import boundaries over 552 files, contrast over 117
       pairs in both themes, colour literals over 557 files with 4 documented exclusions).
     * npm run build: not separately measured, but typecheck is its gate and typecheck is clean.
     * Measured in this worktree on branch spec/289-bonded-return at generation time, with
       node_modules installed. There is nothing pre-existing for you to fix or work around.
6. Proof checkboxes marked OWNER, manual-smoke, or needing a live backend are NOT yours: leave them
   ...
   that is ticket 295 in its entirety - excluded from this run, because it needs a live SIS.Api and the
   BackOffice half to have shipped - plus any Proof box asking a human to eyeball a rendering.
   Note carefully that NO Proof box in tickets 290-294 needs a live backend: every one of them is
   provable against the checked-in fixtures, so an unticked box in 290-294 needs a written reason. The ticket may still complete AFK with those open; every OTHER
...
"@
    ...
    if ($leftBehind) {
        ...
        Say "    .\run-implements-spec289.ps1 -Tickets $(($Tickets | Where-Object { $_ -ne $t }) -join ',')" "Yellow"
        exit 9
    }
...
You are reviewing ONLY the diff introduced by ticket $t of spec 289: git diff $baseSha...HEAD.
...
- The wave's own rules to check against:
  no new npm dependency, feature folder, route or menu entry; canReturn and returnedQuantity are
  OPTIONAL model fields that must fail CLOSED when absent, and returnedQuantity's spelling is a
  deliberate unconfirmed transcription that must NOT have been "corrected"; NO amount of any kind
  on the request body (a whole-body assertion is the wave's most valuable test); header condition
  money is condAmount and never condValue, and the item-0 row is never summed with its per-line
  copies; no BZ02 and no 'BB' literal survives anywhere in src/; i18n keys confined to
  returnDocument.* in the existing document namespace, with beyondBorderOnly and
  returnDocument.unavailable deleted; the ruled decisions are not open - dialog not route, stacked
  not tabbed, no reason pre-selected, address ABSENT (not disabled) under RF, no grand total, no
  box count or weight, fees unticked with no select-all, replay renders as success, refusal keeps
  the dialog open with selections intact, requestId minted per dialog opening not per submit,
  screen stays put on success, no pre-confirm; and tools/document-actions-drive.mjs must have been
  updated in 290 rather than left asserting the deleted gate.
...
Write-Host ""
Say "Spec 289 wave complete. Read .afk\REVIEW-*.md and .afk\HITL-*.md before trusting the results." "Green"
Say ">>> Ticket 295 (the live door) was EXCLUDED from this run: it needs a live SIS.Api and BackOffice spec 1283's tickets to have shipped. It is this wave's drift detector - run it by hand once both are up. <<<" "Yellow"
Say ">>> 295's FIRST job is to confirm what BackOffice spec 1283 section 2b's returned/remaining field is actually CALLED. This wave transcribed 'returnedQuantity' as a GUESS. A mismatch is corrected in 1283 first, never in this repo alone. <<<" "Yellow"
Say ">>> The BackOffice half of this effort (spec 1283's own tickets) runs from its own loop in C:\Work\DMSCO\BackOffice - this runner did not touch that repo. <<<" "Yellow"
Say ">>> Eyeball the dialog in Arabic before shipping. No gate catches a physical Tailwind utility, and this wave adds a whole screen's worth of markup. <<<" "Yellow"
Say ">>> Read .afk\HITL-*.md before trusting anything. The fee-category display constant and the returnedQuantity spelling are the two calls most likely to need a second look. <<<" "Yellow"
Say ">>> This wave ran in the worktree C:\Playground\oms-react-289 on branch spec/289-bonded-return. Nothing was pushed; main is untouched. <<<" "Yellow\"
