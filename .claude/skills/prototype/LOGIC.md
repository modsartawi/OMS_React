...
Add a script to `package.json` (`"proto:<name>": "node <path>"` or a `tsx`-run TS file). The user should run `npm run proto:<name>` — never need to remember a path.

A logic TUI is a small standalone `node` script (or `tsx` for TypeScript) — it does **not** need Vite or the React app; keep the pure logic module importable so it can later lift into `src/` unchanged.
