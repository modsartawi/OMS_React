# Runs /implement + /standards-review for the spec 282 open-settlements tickets ("the estate's
# late money gets a screen to be chased on") in fresh Claude Code sessions, sequentially, fully AFK.
#
#   .\run-implements-spec282.ps1                  (283, 284, 285, 288, 286, 287)
#   .\run-implements-spec282.ps1 -Tickets 286,287
#   .\run-implements-spec282.ps1 -DryRun          (pre-flight + plan only, starts nothing)
#   .\run-implements-spec282.ps1 -SkipReview      (implement rounds only)
#   .\run-implements-spec282.ps1 -SmokeTest       (one trivial session through the real harness)
#
...
#
# ---------------------------------------------------------------------------------------
# Dependency map (spec 282; nothing in this wave has landed yet - 283 is slice 0):
#   283 --+-- 284
#         |
#         +-- 285 --+-- 286
#                   +-- 287
...
#
# Running order:
#   283, 284, 285, 288, 286, 287
#
#   * 284 runs SECOND although it and 285 are siblings off 283. Both 283 and 284 are
#     shared-surface changes - 283 rewrites addresses.ts, the router and SettlementPage's body,
#     284 rewrites AppShell's menu rendering and menu-model's matcher - and shared surfaces go
#     first, or 285 builds a screen on a route table and a nav that are about to move under it.
#   * 288 runs BEFORE 286 and 287 even though it is the smallest slice, because it is the only
#     one of the three that calls no unbuilt door. It reuses 285's answer and adds a signpost.
#   * 286 and 287 run LAST, in that order. Both are backend-blocked: 286 needs
#     Settlement/Uncollected (draft section 2) and 287 needs the chase table plus its write door
#     (draft section 7), neither of which exists. They are therefore the likeliest to burn a
#     session, and a blocker there costs one ticket rather than the night. 287 is dead last: it
#     is the wave's only NEW TABLE, and the two read-only lanes are designed to ship without it.
#
# Preconditions (the script checks 1, 2 and 4 - you own 3 and 5):
#   1. Nothing outside this wave blocks it. Every blocking edge is internal (283 -> 284/285 ->
#      286/287/288) and the loop honours it by running in the order above.
...
#      Expected for this wave: spec/282-open-settlements
...
#   5. A live SIS.Api on :5111 is NOT required and will not help - the three doors this wave
#      needs are not built on the server at all, so every slice stubs the network at Playwright.
#      The prototype at .afk\PROTO-281-open-settlements.html is APPROVED and governs the layout;
#      the loop follows it and does not re-decide the arrangement.
param(
    [int[]]$Tickets = @(283, 284, 285, 288, 286, 287),
    [string]$Model = "opus",
...
Say "Branch: $branch   (expected for this wave: spec/282-open-settlements)" "DarkGray"
...
3. Wave-specific facts - read these before you code:
   - The spec is .issues\282-open-settlements-spec.md and the wayfinder map is
     .issues\275-settlement-follow-up-lanes-map.md. Read the spec AND your ticket.
     NOTHING in this wave has landed yet - 283 is slice 0. Whatever ran before you in this
     loop IS on the branch, so read the committed diffs of 283 onwards before re-deciding
     anything they settled.
   - THE PROTOTYPE GOVERNS THE LAYOUT: .afk\PROTO-281-open-settlements.html is approved and
     is the arrangement, drawn at estate scale. Open it before you build a screen. Do not
     invent a reasonable-but-different layout, and do not restyle it.
   - This wave adds NO npm dependency. If a slice looks like it needs one, that is a BLOCKER,
     not a decision you take at 3am. In particular it does NOT add AG Grid plugins, a date
     library, or a virtualiser.
   - EVERY key goes in the EXISTING `settlement` i18n namespace. Do not mint a namespace. The
     four menu keys are menu.overview / menu.open / menu.ledger / menu.upload; the group header
     above them stays `collection`'s. "Open settlements" is the owner's own wording - use it
     verbatim, do not improve it to "Open entries" or "Outstanding settlements".
   - REVERSAL, and it is the one most likely to be got wrong because the losing wording is
     still sitting in an older ticket: ticket 276 suggests the cash-waiting door might read
     PosSettlementDocument. Ticket 277 OVERTURNED that - the document table is STORE-side and
     `Status = Prepared` selects the empty set at head office. The door reads the consumption
     journal. If you are in 286 and find yourself modelling a document row, stop and re-read 277.
   - REVERSAL: the map says the list is "ranked mine-first". Ticket 281 refined that to two
     SECTIONS (Yours, then Everyone else's), each oldest-first - NOT a sort key, and NOT a
     filter that hides the estate. A single list sorted by isMine is the wrong answer and it
     was specifically rejected: it buries the estate's oldest entry 176 rows down at estate scale.
   - 285 OWNS the lane's pure projection module, its columns and its grid. 286, 287 and 288
     EXTEND it - a third tab, a column, a signpost. None of them may restructure it. If 286
     finds itself rewriting the projection's shape, that is a sign it is doing 285's job again.
   - The server's ORDER IS AUTHORITATIVE and the client must not re-sort. The answer is capped,
     so re-sorting a capped page changes which rows the cap kept. Sort on the wire (sort=age),
     never in the component.
   - SETTLED, do not re-litigate: nothing colours the age (no red, no badge, no "overdue") -
     the domain has not ruled when an entry is late and a colour is a ruling. Money renders
     min-2/max-3 decimals and NO column is totalled (figures.ts refuses to add across
     currencies). No database index is requested anywhere in this wave.
   - SETTLED: cap.ts gets a NEW OPEN_LANE_LIMIT = 2000 and LEDGER_LIMIT STAYS 500. They are two
     constants on purpose - one answers a population, the other answers a question. Do not
     "tidy" them into one.
   - 283 DELETES the `view` query parameter. No later slice may reintroduce a view/mode/tab
     discriminator in the query string to decide WHICH SCREEN draws - the path does that now.
     (`?tab=` on the lane is fine: it selects what one screen is looking at.)
   - 284 is the only slice that touches src\layout. Its blast radius is every nav group in the
     app, so after it, confirm an ordinary two-level group (Collections' four inquiries, or
     Reports) still renders and highlights exactly as before.
   - The chase cell is a TRI-STATE and collapsing two of its cases is a factual error, not a
     style choice: field absent => render NO column at all; null => "Never chased" as a named
     state; otherwise the note. Rendering "Never chased" because the door is missing states,
     confidently, that nobody has chased any of 1394 branches.
   - What "proven" means per slice: 283, 284, 285 are pure-module first (vitest) plus a drive;
     286 and 287 are backend-blocked, so stub the network at Playwright against the shapes
     below and leave any Proof box that genuinely needs the live door unticked and explained.
   - THE SERVER HALF LIVES IN THE OTHER REPOSITORY (C:\Work\DMSCO\BackOffice). You may READ it
     and the draft at .afk\BACKOFFICE-TICKET-DRAFT-settlement-reads.md. You may NOT edit, stage,
   ...
   - NONE of the three doors below is built. Stub these shapes EXACTLY at Playwright to prove the
     screen. Do not invent fields, do not soften a shape to whatever is convenient - a screen
     built against a guessed shape fails the day the endpoint lands, and it fails silently on
     precisely the fields you guessed.
     (285, draft section 6) GET Settlement/Ledger?status=OPEN&sort=age&limit=2000
       rows are today's SettlementLedgerRow PLUS three OPTIONAL fields:
         servedBy (string), isMine (bool), ageDays (int), lastChase (object or null, see below)
       Optional means: when they are absent the screen renders ONE unsectioned oldest-first list
       and derives NOTHING. It must never compute an age from a local clock or guess a ranking.
     (286, draft section 2) GET Settlement/Uncollected?limit=500
       ...
       Estate-wide, NO scope parameter. There is no documentNumber - it is '' by definition.
     (287, draft section 7) POST Settlement/Chase
       body { storeId, subject, subjectId, entryNumber, note }  subject = BRANCH | ENTRY | RECEIPT
       reply { accepted, chase }   -- a refusal is 200 with accepted:false, NOT an error status
       lastChase on a lane row is { at, by, note } or null; ABSENT is the third case.
...
5. How to verify - a green typecheck is NOT proof a screen works:
   ...
   - A UI slice is proven by DRIVING THE APP with a Playwright script under tools\*-drive.mjs.
     ...
     rather than inventing a new harness. tools\settlement-drive.mjs is THIS screen's drive -
   ...
   - Pre-existing baseline for this wave, measured when this runner was generated -
     ...
     * npm test: CLEAN - 1828 tests across 116 files, all passing. A red test is yours.
     * npm run lint: all three gates CLEAN (541 files boundaries, 117 contrast pairs, 546 files
       colour literals). A new failure is yours.
6. Proof checkboxes marked OWNER, manual-smoke, or needing a live backend are NOT yours: leave them
   ...
   that is any box needing a LIVE Settlement/Uncollected (286), a live Settlement/Chase (287), the
   live ranking fields on Settlement/Ledger (285), and any box asking a human to eyeball the Arabic
   rendering. The ticket may still complete AFK with those open; every OTHER
...
"@
    ...
    if ($leftBehind) {
        ...
        Say "    .\run-implements-spec282.ps1 -Tickets $(($Tickets | Where-Object { $_ -ne $t }) -join ',')" "Yellow"
        exit 9
    }
...
You are reviewing ONLY the diff introduced by ticket $t of spec 282: git diff $baseSha...HEAD.
...
- The wave's own rules to check against:
  no new npm dependency; every key in the EXISTING `settlement` namespace; the prototype at
  .afk\PROTO-281-open-settlements.html governs the layout; NOTHING colours the age and no money
  column is totalled; the client never re-sorts a capped answer and never derives an age from a
  local clock; mine/theirs are two SECTIONS, not a sort key or a filter that hides the estate;
  the chase cell keeps three distinct cases (absent => no column, null => "Never chased", else
  the note) and collapsing two of them is a factual error; OPEN_LANE_LIMIT and LEDGER_LIMIT stay
  two constants; no slice after 283 reintroduces a query parameter that decides which screen
  draws; 285 owns the projection and 286/287/288 only extend it; backend-blocked Proof boxes
  must be honest, not faked green.
...
Write-Host ""
Say "Spec 282 wave complete. Read .afk\REVIEW-*.md and .afk\HITL-*.md before trusting the results." "Green"
Say ">>> Still outstanding: NONE of the three server doors exist. 285 renders against today's ledger (no ranking), 286 and 287 are fixture-only until BackOffice builds sections 2, 6 and 7. <<<" "Yellow"
Say ">>> The BackOffice asks are still a DRAFT at .afk\BACKOFFICE-TICKET-DRAFT-settlement-reads.md and have never been minted as issue 1199 in C:\Work\DMSCO\BackOffice - this runner did not touch that repo. <<<" "Yellow"
Say ">>> Eyeball the Arabic rendering of the new lane before shipping; no gate catches a physical Tailwind utility, and this wave adds a whole grid. <<<" "Yellow"
Say ">>> This wave built on branch spec/282-open-settlements - merge it back to main when you are happy with the reviews. <<<" "Yellow"
