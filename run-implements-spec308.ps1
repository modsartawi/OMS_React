# Runs /implement + /standards-review for spec 308 (the web half of BackOffice spec 1976 - the collection cycle) in fresh Claude Code sessions,
# sequentially, fully AFK.
#
#   .\run-implements-spec308.ps1                  (309, 310, 311, 312, 313, 315, 316, 317, 318, 314)
#   .\run-implements-spec308.ps1 -Tickets 315,316
#   .\run-implements-spec308.ps1 -DryRun          (pre-flight + plan only, starts nothing)
#   .\run-implements-spec308.ps1 -SkipReview      (implement rounds only)
#   .\run-implements-spec308.ps1 -SmokeTest       (one trivial session through the real harness)
#
...
#
# ---------------------------------------------------------------------------------------
# Dependency map (local blocked-by edges; each ticket ALSO waits on its BackOffice ticket's Web contract):
#   309 --> 310          315 --+-- 316
#   311, 312, 313, 318         \-- 317
#   314 (touches every grid)
#   BackOffice: 309<-1977+1978  310<-1979  311<-1980  312<-1984  313<-1987  314<-1990
#               315<-1992  316<-1993  317<-1994+1995  318<-1996
#
# Running order:
#   * The order MIRRORS the BackOffice runner's order (spec1976), so a BackOffice wave that stopped
#     part-way still lets the React tickets whose contracts exist run first, and stops at the first
#     one whose contract does not (AFK-BLOCKED, exit 2) - that is the intended stop, not a fault.
#   * 315 runs before 316 and 317: it sets the four-filter criteria shape both copy.
#   * 314 runs LAST: it adds a profit center column to every grid 315-317 just reshaped, and its
#     BackOffice twin (1990) runs last there too.
#
# Preconditions (the script checks 1, 2 and 4 - you own 3 and 5):
#   1. Nothing local blocks it - but it is meant to run AFTER the BackOffice spec1976 wave has landed (see 5).
...
#      Expected for this wave: spec308
...
#   5. Run this AFTER C:\Work\DMSCO\BackOffice-spec1976's runner (1977-1996). Each
#      session reads its BackOffice ticket's '## Web contract' there. No live SIS.Api is needed:
#      every slice stubs the network at Playwright against that contract.
param(
    [int[]]$Tickets = @(309, 310, 311, 312, 313, 315, 316, 317, 318, 314),
    [string]$Model = "opus",
...
Say "Branch: $branch   (expected for this wave: spec308)" "DarkGray"
...
3. Wave-specific facts - read these before you code:
   - The spec is .issues\308-collection-cycle-web-half-spec.md and the wayfinder map is
     C:\Work\DMSCO\BackOffice\.issues\1976-collection-cycle-roles-papers-and-collector-guards-spec.md (the governing BackOffice spec; no wayfinder map). Read the spec AND your ticket.
     No ticket of this wave has landed before this run; read the committed diffs of the ones that
     ran before yours in THIS run (git log on spec308) before re-deciding anything they settled.
   - This wave adds NO npm dependency and NO new area. Everything extends features\collection\
     (inquiry and settlement). If a slice seems to need a package, that is a BLOCKER.
   - The Ready screen (317) lives in features\collection\inquiry beside its siblings: same nav group,
     same /collection/* prefix, same ScreenGate (it is in @/core/ui already), same Served-by
     control. Do not make a new area for it.
   - New access booleans (settlement supervision, the Ready grant) join CollectionAccessResult in
     src\core\models\collection.ts. Read the canOpenSettlement comment there: a grant the contract
     owes is REQUIRED and read as === true - never ?? true, never optional-as-allowed.
   - The web is ENGLISH-ONLY (zero-literal, en bundles collection.json / settlement.json). Where a
     BackOffice ticket says EN+AR, that is the server's refusal envelope and the till; the web shows
     the server's message or its own en key, and adds no Arabic bundle.
   - 309: headline totals and every figure/projection module must EXCLUDE pending and rejected
     entries; keep 269's rule that the signed position is displayed, never consumed. Approve and
     Reject show only on a pending row and only with the supervision boolean; Reject needs a reason.
   - 310: the correction actions (EntryCorrection, BatchWithdraw) disappear WITHOUT the supervision
     boolean; the server's 403 is the real guard, the hiding is courtesy - handle the 403 anyway.
   - 312: the red box is .cv-overage-box in CollectionVoucher; it must wrap 200 characters on both
     page kinds and still print on A4 (collection-print-drive.mjs is the precedent drive).
   - 314: render the profit center exactly as the server sends it. Do NOT format or derive it
     client-side (no PH- prefixing, no SAP resolver copy) - the formatter is server-side by ruling.
   - 315/316: criteria modules keep PascalCase wire keys and DROP empty values (never send '');
     date ranges are inclusive by day; Collections still lands on today..today by collection
     date. 316 adds 'attempts' to the ServedByScreen table in served-by.ts and pins it in
     served-by.test.ts - the per-screen ruling lives in that one table by design.
   - 318: copy the settlement bulk upload's shape (UploadView / BulkUploadDialog / bulk-template):
     preview, then commit re-sends the same file; a re-press must not apply twice.
   - Settlement/Chase is called by the web but not mapped on the server - not this wave's.
   - THE CONTRACTS LIVE IN THE OTHER REPOSITORY: C:\Work\DMSCO\BackOffice (branch main).
     The spec1976 wave has MERGED to main there (merge ae11fac70) and its worktree is gone: every
     link in a ticket or the spec to C:\Work\DMSCO\BackOffice-spec1976\... means the SAME path under
     C:\Work\DMSCO\BackOffice\... - read it there. A dead spec1976 link is NOT a missing contract.
     You may READ its .issues, docs\adr and source (Services\SIS.Api, Sartawi.Retail.Data). You may
     NOT edit, stage, commit, build or run anything there - it has its own loop and reviewer.
   - FIRST ACT OF EVERY TICKET: open the BackOffice ticket(s) your ticket names under 'Blocked by'.
     Each must be 'status: done' AND carry a '## Web contract' section. If either is missing, that
     is a BLOCKER (the endpoint shape does not exist yet): log it and stop with AFK-BLOCKED. Do not
     guess a shape - a screen built against a guessed shape fails silently on the guessed fields.
   - Build against the Web contract VERBATIM, stubbed at Playwright. Cross-check it against the
     committed SIS.Api handler in that tree; if they disagree, follow the CODE (it is what will be
     served), log the difference in the HITL doc, and say so in your ticket.
...
6. How to verify - a green typecheck is NOT proof a screen works:
   ...
   - Pre-existing baseline for this wave, measured when this runner was generated -
     ...
     * npm test: CLEAN - 134 files, 2226 tests, all passing. Any failure is yours.
     * npm run lint: all three gates CLEAN (boundaries, contrast, palette).
7. Proof checkboxes marked OWNER, manual-smoke, or needing a live backend are NOT yours: leave them
   ...
   that is any drive against a LIVE SIS.Api (every slice stubs instead), a human eye on the printed A4
   voucher's red box (312), and any Proof box whose BackOffice contract was missing (that ticket stops
   AFK-BLOCKED instead). The ticket may still complete AFK with those open; every OTHER
...
"@
    ...
    if ($leftBehind) {
        ...
        Say "    .\run-implements-spec308.ps1 -Tickets $(($Tickets | Where-Object { $_ -ne $t }) -join ',')" "Yellow"
        exit 9
    }
...
You are reviewing ONLY the diff introduced by ticket $t of spec 308: git diff $baseSha...HEAD.
...
- The wave's own rules to check against:
  no new npm dependency and no new area; every server field used must appear in the BackOffice
  ticket's Web contract (or its committed handler) - an invented field is SERIOUS; pending/rejected
  entries never reach a total; supervision-only actions are hidden without the boolean AND the 403 is
  handled; new access booleans are required and read === true; profit center is rendered, never
  derived; criteria drop empty values and keep PascalCase keys; web stays en-only; nothing in the
  BackOffice repo was touched; live-backend Proof boxes are left honest, not faked.
...
Write-Host ""
Say "Spec 308 wave complete. Read .afk\REVIEW-*.md and .afk\HITL-*.md before trusting the results." "Green"
Say ">>> Nothing here is live until the BackOffice spec1976 wave merges and its SQL, seeds and SIS.Api deploy (BackOffice 1997 cutover). <<<" "Yellow"
Say ">>> Drive each screen once against a real SIS.Api after that deploy; tonight every proof was stubbed against the Web contracts. <<<" "Yellow"
Say ">>> Print one voucher with a 200-character description on real A4 and look at the red box (312). <<<" "Yellow"
Say ">>> Merge back: rebase spec308 onto main, then git -C C:\Playground\oms-react merge --ff-only spec308. Copy .afk\HITL-*.md out BEFORE any git worktree remove. <<<" "Yellow"
