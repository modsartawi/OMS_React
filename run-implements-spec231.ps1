# Runs /implement + /standards-review for the spec 231 Loy member lookup tickets ("one field resolves a member") in fresh Claude Code sessions,
# sequentially, fully AFK.
#
#   .\run-implements-spec231.ps1                  (232, 233, 234, 235, 236, 237, 238)
#   .\run-implements-spec231.ps1 -Tickets 236,237
#   .\run-implements-spec231.ps1 -DryRun          (pre-flight + plan only, starts nothing)
#   .\run-implements-spec231.ps1 -SkipReview      (implement rounds only)
#   .\run-implements-spec231.ps1 -SmokeTest       (one trivial session through the real harness)
#
...
#
# ---------------------------------------------------------------------------------------
# Dependency map (231 spec; nothing landed yet - this is the whole wave):
#   232 (prefactor, independent) -------------------+
#                                                   |
#   233 --+-- 234                                   |
#         +-- 235                                   |
#         +-- 236 --+-- 237                         |
#                   +-- 238 <----------------------+
#
#   232 gates 238 only. 233 gates everything else.
#
# Running order:
#   * Topological, with one force applied: 232 runs FIRST even though only 238 depends on it.
#     It is a shared-surface change - it moves GridPager and its pager.* i18n keys OUT of
#     features/admin/ua-admin and INTO core. Any slice running before it that touched a pager
#     would carry a doomed import path through its own diff, and the boundary gate would fail
#     the build rather than warn.
#   * "Backend-blocked slices run last" is deliberately NOT used to reorder here. All seven are
#     equally backend-blocked: the LoyWeb/* door does not exist in any environment, and every
#     slice is specced to be proven against stubs. There is no less-blocked slice to promote,
#     so promoting one would only shuffle risk without reducing it.
#   * 236 before 237 and 238 is more than the graph. 236 builds the tab shell - lazy fetch, the
#     ?tab= param, the ceiling caption, the per-tab empty sentence and the scoped-Retry failure
#     surface - that both later tabs slot into. Built second, either would ship its own shell
#     and 236 would then rewrite it.
#
# Preconditions (the script checks 1, 2 and 4 - you own 3 and 5):
#   1. Nothing outside this wave blocks it - map 222's eight decision tickets (223-230) are all done and committed at bc1f695.
...
#   5. A live SIS.Api on :5111 is NOT required and would NOT help - the LoyWeb/* door every slice calls does not exist in any environment. Every slice stubs the network at Playwright. The layout is APPROVED and drawn: .issues\assets\227-member-screen.PROTOTYPE.html, variant B - the loop follows it and does not re-decide layout.
param(
    [int[]]$Tickets = @(232,233,234,235,236,237,238),
    [string]$Model = "opus",
...
3. Wave-specific facts - read these before you code:
   - The spec is .issues\231-loy-member-lookup-spec.md and the wayfinder map is
     .issues\222-a-loy-member-read-only-in-the-portal.md. Read the spec AND your ticket.
     Nothing in this wave has landed yet - there are no sibling diffs to read. What
     exists instead are the EIGHT DECISION TICKETS behind the spec (223-230), all done: they are
     where every ruling below was argued and settled. Read the one your slice touches instead of
     re-deriving it, and read 227 (the layout) before you render anything.
   - This wave adds NO npm dependency. Not for a date library, not for a currency formatter, not
     for a tab component. If a slice looks like it needs one, that is a BLOCKER, not a decision
     you take at 3am.
   - This wave creates a NEW AREA, features/loy/, the first since the area taxonomy rule. Its
     feature folder is features/loy/member/. Registration is the standard four points: the
     folder, the namespace in core/i18n.ts, the routes in app/router.tsx, the menu item in
     layout/menu-model.ts.
   - The i18n namespace is `loy`, NOT `member`. This is a DELIBERATE deviation from the standing
     "namespace == feature name" rule: the feature is `member` under the `loy` area, and `member`
     is far too generic as a global namespace. Tickets 229 and 230 already fixed t('loy:...')
     call sites. Do not "correct" it to `member`.
   - Every key this wave writes lives under the loy namespace. A key invented outside it passes
     every gate and is invisible until someone translates the app.

   - TRAP (232): GridPager CANNOT be reused where it stands, and the spec prose that says
     "the existing GridPager" is what 232 exists to correct. Four separate problems, all
     verified in the code: (a) it lives in features/admin/ua-admin, so a features/loy import
     FAILS npm run lint's boundary gate; (b) PAGE_SIZE is hardcoded 50, Loy Actions needs 25;
     (c) pagerButtonEnablement drives Next off `isCapped` ("more rows beyond THIS page", a
     Ua-users envelope flag) but the Loy actions read returns a REAL recordsCount, so Next must
     come from page < pageCount; (d) the component hardcodes useTranslation('ua-admin').
     232 must ADD the next-from-total path beside next-from-capped. Do not replace the capped
     path - Ua Users still needs it, and Ua Users must come out observably unchanged.

   - TRAP: `formatStamp` DOES NOT EXIST, despite that name appearing in tickets 226 and 231.
     The real formatters in core/util/date-format.ts are formatDateTime (yyyy-MM-dd HH:mm) and
     formatShortDate (25 Jul 2026). Both already cover this wave. Do not write a date module.
   - TRAP: isBlankDate ALREADY EXISTS in the same file, already handles the 0001-01-01 sentinel,
     and is exported specifically so a second spelling of "unset" never appears. The member's
     birthDate guard is a REUSE of it, not a new helper.
   - TRAP: formatMoney in core/util/number-format.ts is documented as "the single money
     formatter for the app" and is FIXED 2dp. Sales needs per-currency decimals (BHD is 3).
     Write a NEW currency-aware pure function INSIDE features/loy - do NOT widen formatMoney,
     because every one of its existing callers means 2dp.

   - THE HEADLINE CORRECTNESS RULE (233): in resolveMember, ONLY a business LOY-00100 may
     cascade from the mobile call to the LoyId call. An ungranted portal call returns a BARE
     403. A cascade-on-anything rule renders the shut door - and every outage - as "no member
     matches", doubling load on a dead API while telling the agent to re-read a number that was
     correct. auth / server / network must show themselves and stop. This is the single defect
     this wave most needs not to ship.
   - The field COMPACTS to digits (whitespace, dashes, parens, leading +) and does nothing else.
     No dialling code, no PadLeft, no country rule. The rule that builds the loyalty key is
     SERVER-side. A client that prefixes 966 is wrong even though it will appear to work.
   - The search box is NEVER rewritten after a search. A double miss is a neutral CLIENT
     sentence in the empty state, not a server message - this knowingly bends api-envelope, and
     it was argued and ruled in 225. Do not "fix" it back to surfacing the server text.
   - Submit is EXPLICIT (Enter or the button). Blank submit is silent. Do not add debounced
     as-you-type search.

   - REVERSAL (226 corrected 223, and 223's asset still contains the old wording): activity
     Points arrive ALREADY SIGNED from the server. Do NOT build a client-side debit/credit
     table and do NOT derive the sign from ActivityType. 223 says to; 226 struck it.
   - REVERSAL (same pair): Sales money is MULTI-CURRENCY. 223 implies 2dp everywhere; Bahrain
     BHD stores are live and BHD is the only 3-decimal currency in the footprint. "Always 2dp"
     holds for POINTS and not for money.
   - REVERSAL (230 amended 228): the archived-member constraint on 225 DISSOLVED. Both keys
     resolve the same member. LOY-00101 survives only as a defensive guard in resolveMember,
     expected unreachable - its test exists to say so. Do not build an archived error path.

   - SETTLED, do not re-litigate: no lookup calls (Loy/Tiers and Loy/MemberBlockedReasons are
     routes five and six and the door stays at four); no export from any tab; no row links
     anywhere (checked - no route accepts a TrxNumber, ActivityId or ActionNo, and
     oms/document/:documentNo is a different identifier space that would 404); no member picker
     or disambiguation UI (one mobile resolves to at most one member by construction); no RTL
     test runner bootstrap.
   - SETTLED: an ordinary unblocked member shows NO status chip at all. Silence IS the Active
     state. Do not add an "Active" chip - that shape was proposed and rejected.
   - SETTLED: activity points are NEVER coloured. The Activity column already names the
     direction in the server's own English, so colour would be a second reading of a stated
     fact - and the only reading for anyone who cannot see it.
   - SETTLED: no client-side total on Activities. The server rounds each row, so summing the
     rounded rows will NOT equal the header pointsBalance. A total is a visible wrong number.
   - SETTLED: capped tabs state their ceiling and NEVER a bare row count (a count reads as
     completeness). Actions is the opposite case and states its real total.
   - SETTLED: sort and filter are ON for Activities and Sales (their whole window is in the
     browser) and OFF for Actions (sorting page 3 of N reorders a page, not a result). This
     line is principled, not an inconsistency to tidy.
   - SECURITY: the Actions read MUST always carry LoyId. Called bare it returns the first 25
     actions of the WHOLE ESTATE, across all members - a silent cross-member leak, not an error.
     Pin it with a test even though the door is also supposed to enforce it.
   - PII: the Actions row drops the ENTIRE member snapshot (Mobile, FullName, Email, Gender,
     CityName, InsuranceCompany, BlockedReason, JoinedDate). Do not add them back as "useful
     context" - it is the member already on screen, repeated 25 times a page.

   - DESIGN FIDELITY: .issues\assets\227-member-screen.PROTOTYPE.html GOVERNS. It is a working
     page in both themes and it carries the two REJECTED variants as well as the taken one -
     open it before building the header or the tab strip. Variant B is the one that ships. Do
     not invent a reasonable-but-different arrangement; it will be reverted.
   - WPF is a SOURCE, NOT A TARGET. C:\Work\DMSCO\BackOffice\Sartawi.Retail\IC supplies the
     data and the domain rules and NEVER the layout, the control choices or the interaction.
     "Because that is what WPF does" is not an answer any slice in this wave may give.

   - WHAT "PROVEN" MEANS HERE: the pure modules (resolve-member, codes, member-header,
     tab-volume, sales-columns, the pager arithmetic, the money formatter) carry the wave and
     are proven by vitest. The screens are proven by extending tools/loy-member-drive.mjs
     against STUBBED envelopes on port 5199. React Testing Library is NOT installed and this
     wave does NOT bootstrap it.
   - Each slice EXTENDS the drive rather than deferring capture to a final pass. There is no
     hardening ticket at the end of this wave to catch what you skipped.
   - The SERVER half of this effort lives in the OTHER repository
     (C:\Work\DMSCO\BackOffice, issues 977-979: the gate, the four reads, the probe). You may
     READ those issues and anything they name. You may NOT edit, stage, commit or run anything
     in that repository - it has its own branch, its own loop and its own reviewer. If you
     believe something there is wrong, log it to the HITL doc and carry on with your slice.
   - EVERY endpoint this wave calls is UNBUILT. There is no LoyWeb/* route in any environment,
     so a "let me just check against the real API" detour is not a slow path, it is a dead one.
     The agreed shapes, read off C# classes in 223's field inventory - stub EXACTLY these:

       GET LoyWeb/MemberByMobile/{typed}  -> LoyMemberModel
       GET LoyWeb/Member/{loyId}          -> LoyMemberModel
         envelope: { statusCode, success, message, errors, data }
         a MISS is a business 400: success:false, errorCode LOY-00100, data absent
         LoyMemberModel carries: loyId, mobile, mobileCountry, fullName, birthDate, gender,
           email, nationality, nationalId, insuranceCompany, cityCode, preferredLanguage,
           joinDate, lastUpdate, tier, tierPointsBalance, pendingPoints, pointsBalance,
           pointsBalanceAmount, pointsBalanceAmountCurrency, pointsExpireSoon,
           pointsExpireSoonDays, exchangeRate, profileUpdated, profile, accrualFactor,
           redemptionFactor, blockedReasonCode, and memberType (added by door issue 978).
         C# has NO nullable annotations here: treat EVERY string as nullable in TypeScript.
         birthDate is NOT nullable - an unset one arrives as the 0001-01-01 sentinel.

       GET LoyWeb/Reports/LastActivities/{loyId}  -> array, TOP 100, no total
       GET LoyWeb/Reports/LoyaltySales/{loyId}    -> array, TOP 500, no total
         BOTH are raw SQL with NO existence check: a missing member is 200 with [], never a
         refusal. Only the MEMBER call can refuse a bad key.

       GET LoyWeb/Reports/LoyMemberActions (+ LoyId, paging) -> paged, REAL recordsCount

       GET LoyWeb/Access -> the probe. Cookie-only, NOT grant-gated. An ungranted session
         answers DENIED WITH A 200, not a 403 - the portal fails closed on it either way.

     Do not invent fields and do not soften a shape to whatever is convenient. A screen built
     against a guessed shape fails the day the endpoint lands, and it fails silently on exactly
     the fields you guessed.
   - branchId is NEVER passed on any of these calls. It only restates a points figure in a
     non-SAR plant currency, every KSA branch is SAR, and passing it just widens the cache key.
...
5. How to verify - a green typecheck is NOT proof a screen works:
   ...
   - Pre-existing baseline for this wave, measured when this runner was generated -
     ...
     * npm test: 1085 tests GREEN across 64 files, zero failures. Any red is yours.
     * npm run lint: all three gates CLEAN - 376 files boundaries, 117 contrast pairs across
       both themes, 378 files palette with 4 documented exclusions.
     * Measured 2026-08-06 at HEAD bc1f695, immediately before this runner was generated. The
       tree was committed clean at that point; nothing in this wave predates you.
6. Proof checkboxes marked OWNER, manual-smoke, or needing a live backend are NOT yours: leave them
   ...
   that is EVERY integration-level proof, because the LoyWeb/* door does not exist - so each ticket's closing note must say OUT LOUD that nothing was driven against a live SIS.Api, exactly as the twelve tickets of the Nphies wave did; plus a human's eye on the member header and tab strip against the approved prototype. The ticket may still complete AFK with those open; every OTHER
...
"@
    ...
    if ($leftBehind) {
        ...
        Say "    .\run-implements-spec231.ps1 -Tickets $(($Tickets | Where-Object { $_ -ne $t }) -join ',')" "Yellow"
        exit 9
    }
...
You are reviewing ONLY the diff introduced by ticket $t of spec 231: git diff $baseSha...HEAD.
...
- The wave's own rules to check against:
  no new npm dependency; the LoyWeb/* door does not exist in any environment, so a slice
  claiming a live call is wrong and a backend-blocked Proof box must stay honest rather than
  faked green; ONLY a business LOY-00100 may cascade in resolveMember, and a bare 403 rendering
  as "no member matches" is this wave's headline defect; the field compacts to digits and never
  builds the loyalty key (that rule is server-side); activity points arrive already signed and
  must not be re-signed or coloured client-side; there is no client-side points total; Sales
  money is per-row currency, not fixed 2dp, and formatMoney must not have been widened; an
  ordinary unblocked member shows NO status chip; the Actions read always carries LoyId and
  drops the whole member snapshot; capped tabs state a ceiling and never a bare row count;
  sort/filter ON for the two held tabs and OFF for the paged one; 236 owns the tab shell and
  237/238 must not restructure it; 232 must ADD the next-from-total path without removing
  next-from-capped, and Ua Users must be observably unchanged; i18n keys confined to the loy
  namespace; features/loy may never import another feature; the approved prototype at
  .issues\assets\227-member-screen.PROTOTYPE.html governs the layout, not the reviewer's taste.
...
Write-Host ""
Say "Spec 231 wave complete. Read .afk\REVIEW-*.md and .afk\HITL-*.md before trusting the results." "Green"
Say ">>> Nothing in this wave was driven against a live SIS.Api - the LoyWeb/* door does not exist yet. Every screen is proven against stubs only. <<<" "Yellow"
Say ">>> The server half (BackOffice 977-979: gate, four reads, probe) runs from its own loop in C:\Work\DMSCO\BackOffice - this runner did not touch it. <<<" "Yellow"
Say ">>> Integration is real only when 977-979 land. Re-drive the whole screen against the door then, and expect the mobile-normalisation case to be where it bites. <<<" "Yellow"
Say ">>> Eyeball the member header and tab strip against .issues\assets\227-member-screen.PROTOTYPE.html (variant B) before shipping - no gate catches an invented arrangement. <<<" "Yellow"
Say ">>> Check that /admin/ua-users still pages exactly as before: ticket 232 moved its pager into core, and no test in this wave watches that screen. <<<" "Yellow"
